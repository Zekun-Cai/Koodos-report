...
**TL;DR**: 本研究提出了一种方法，能够在领域数据分布持续变化的动态环境中，基于随机时刻观测的数据分布，在任意时刻生成适用的神经网络，实现前所未有的泛化能力。下图展示了模型在领域数据随时间发生旋转和膨胀时的泛化表现。通过在随机时间点（蓝色标记点）的观测，模型在任意时刻生成了适用的神经网络，其决策边界始终与数据分布变化保持协调一致。  
